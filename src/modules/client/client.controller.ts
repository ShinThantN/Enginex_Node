...
export async function updateProfile(req: Request, res: Response) {
  ...
  const profile = await clientService.updateClientProfile(req.user!.id, parsed.data);
  res.json({ data: profile });
}
...
export async function saveFavorite(req: Request, res: Response) {
  ...
  const favorite = await clientService.saveFavorite(req.user!.id, parsed.data.engineerProfileId);
  if (!favorite) {
    res.status(404).json({ error: "Engineer profile not found" });
    return;
  }
