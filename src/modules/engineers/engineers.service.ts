...
import type { AvailabilityStatus, EngineerSpecialization, Prisma } from '../../../generated/prisma/client.ts';
...
export const setEngineerPrismaForTests = (prismaClient: PrismaClientInstance) => {
...
const buildPortfolioCreateData = (
  ...
  return {
    ...
    ...(portfolio.description !== undefined ? { description: portfolio.description } : {}),
    ...(portfolio.imageUrl !== undefined ? { imageUrl: portfolio.imageUrl } : {}),
    ...(portfolio.projectLink !== undefined ? { projectLink: portfolio.projectLink } : {}),
    ...(portfolio.startDate !== undefined ? { startDate: new Date(portfolio.startDate) } : {}),
    ...(portfolio.endDate !== undefined ? { endDate: new Date(portfolio.endDate) } : {}),
  };
};

const buildUserUpdateData = (profileData: EngineerProfileInput): Prisma.UserUpdateInput => ({
...
const getEngineerSpecialization = (specialization?: string): EngineerSpecialization | undefined => {
  if (specialization === undefined) return undefined;
  if (['CIVIL', 'ARCHITECT', 'MECHANICAL', 'ELECTRICAL'].includes(specialization)) {
...
const buildEngineerProfileCreateData = (
  ...
  return {
    ...
    ...(profileData.yearsOfExperience !== undefined ? { yearsOfExperience: profileData.yearsOfExperience } : {}),
    ...
    ...(profileData.tuVerified !== undefined ? { tuVerified: profileData.tuVerified } : {}),
  };
};

const buildEngineerProfileUpdateData = (profileData: EngineerProfileInput): Prisma.EngineerProfileUncheckedUpdateInput => {
  const specialization = getEngineerSpecialization(profileData.specialization);
...
const buildProjectApplicationData = (
  ...
  status: 'PENDING',
  ...(applicationData.message !== undefined ? { message: applicationData.message } : {}),
  ...(applicationData.proposedPrice !== undefined ? { proposedPrice: applicationData.proposedPrice } : {}),
});
...
export const getEngineerProfileService = async (userId: number) => {
  ...
  if (user.role !== 'ENGINEER') throw new AppError('Engineer profile is only available for ENGINEER users', 403);
...
const getEngineerProfileRecord = async (userId: number) => {
  ...
  if (!user) throw new AppError('User not found', 404);
  if (user.role !== 'ENGINEER') throw new AppError('Engineer operations are only available for ENGINEER users', 403);
  if (!user.engineerProfile) throw new AppError('Engineer profile not found', 404);

  return user.engineerProfile;
};

export const updateEngineerProfileService = async (userId: number, profileData: EngineerProfileInput) => {
  const prisma = await getEngineerPrisma();
...
export const updateEngineerStatusService = async (userId: number, availabilityStatus: AvailabilityStatus) => {
  const prisma = await getEngineerPrisma();
  await getEngineerProfileService(userId);
...
export const applyToProjectService = async (
  ...
  if (project.status !== 'OPEN') throw new AppError('Project is not open for applications', 409);
  if (project.visibility !== 'PUBLIC' || project.assignmentType !== 'OPEN') {
    throw new AppError('Project is not available for solo engineer applications', 403);
  }
  ...
  if (existingApplication) throw new AppError('You have already applied to this project', 409);

  return prisma.projectResponse.create({
    data: buildProjectApplicationData(projectId, engineerProfile.id, applicationData),
    include: {
      project: {
        select: projectSelect,
      },
