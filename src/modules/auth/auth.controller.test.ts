...
app.use((err: TestError, _req: express.Request, res: express.Response) => {
  ...
  res.status(statusCode).json({ success: false, status: statusCode, message: err.message });
});
...
describe('Auth Controller Tests', () => {
  ...
  describe('POST /api/auth/register', () => {
    ...
    it('should return 409 if email already exists', async () => {
      (authService.registerUserService as unknown as AsyncMock<unknown>).mockRejectedValue(
        new authService.AppError('Email already existed', 409)
      );

      const res = await request(app)
        .post('/api/auth/register')
        .send({ name: 'Thura', email: 'thura@gmail.com', password: 'Password123!', role: 'CLIENT' });
    ...
    it('should return 201 and user data on successful registration', async () => {
      ...
      expect(authService.generateToken).toHaveBeenCalledWith(expect.any(Object), '1');
    });
  });
  ...
  describe('POST /api/auth/login', () => {
    ...
    it('should return 401 if user credentials are invalid', async () => {
      ...
      (authService.loginUserService as unknown as AsyncMock<unknown>).mockImplementation(() => {
        throw appError;
      });
    ...
    it('should return 200 and user data on successful login', async () => {
      const mockUser = { id: 1, fullName: 'Thura', email: 'thura@gmail.com', role: 'USER' };
      (authService.loginUserService as unknown as AsyncMock<typeof mockUser>).mockResolvedValue(mockUser);
